...
include { ANNDATA_ATTACHCELLTYPES } from '../../../modules/local/anndata/attachcelltypes'
...
workflow CISTOPIC_ATTACHGEX {
    ...
    mtx_dirs = sample_table
        ...
        .map{ row ->
            def mtx_path = file( "${row.filedir}/filtered_feature_bc_matrix" )
    ...
    versions = ANNDATA_TOH5AD.out.versions.first().mix(
        ANNDATA_ATTACHCELLTYPES.out.versions.first(),
